...
def get_image(picture_id):
    try:
        ...
        if not response:
            response = make_response(jsonify({"error": f"Image with ID {picture_id} not found"}), 404)
            return response

        return jsonify(response)

    except ValueError as value_error:
        response = make_response(jsonify({"error": f"Invalid picture_id: {str(value_error)}"}), 400)
        return response
